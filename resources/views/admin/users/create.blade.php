    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.users.store") }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('google_calendar') ? 'is-invalid' : '' }}" type="text" name="google_calendar" id="google_calendar" value="{{ old('google_calendar', '') }}">
            ...
            <div class="form-group">
                <label for="disabled_work_types">{{ trans('cruds.user.fields.disabled_work_types') }}</label>
                <div style="padding-bottom: 4px">
                    <span class="btn btn-info btn-xs select-all" style="border-radius: 0">{{ trans('global.select_all') }}</span>
                    <span class="btn btn-info btn-xs deselect-all" style="border-radius: 0">{{ trans('global.deselect_all') }}</span>
                </div>
                <select class="form-control select2 {{ $errors->has('disabled_work_types') ? 'is-invalid' : '' }}" name="disabled_work_types[]" id="disabled_work_types" multiple>
                    @foreach($disabled_work_types as $id => $disabled_work_types)
                        <option value="{{ $id }}" {{ in_array($id, old('disabled_work_types', [])) ? 'selected' : '' }}>{{ $disabled_work_types }}</option>
                    @endforeach
                </select>
                @if($errors->has('disabled_work_types'))
                    <span class="text-danger">{{ $errors->first('disabled_work_types') }}</span>
                @endif
                <span class="help-block">{{ trans('cruds.user.fields.disabled_work_types_helper') }}</span>
            </div>
            <div class="form-group">
