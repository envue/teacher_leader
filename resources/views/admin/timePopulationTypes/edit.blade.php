...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.timePopulationType.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.time-population-types.update", [$timePopulationType->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label class="required" for="name">{{ trans('cruds.timePopulationType.fields.name') }}</label>
                <input class="form-control {{ $errors->has('name') ? 'is-invalid' : '' }}" type="text" name="name" id="name" value="{{ old('name', $timePopulationType->name) }}" required>
                @if($errors->has('name'))
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('system_value') ? 'is-invalid' : '' }}">
                    ...
                    <input class="form-check-input" type="checkbox" name="system_value" id="system_value" value="1" {{ $timePopulationType->system_value || old('system_value', 0) === 1 ? 'checked' : '' }}>
